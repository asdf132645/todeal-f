    ...
    <v-card>
      ...
      <v-card-text style="max-height: 500px; overflow-y: auto;">
        <template v-if="type === 'terms'">
          ...
          <p class="mb-2">{{ $t('auto_key_208') }}</p>
          <p class="text-body-2 mb-4">
            본 약관은 대한민국 법률에 따르며, 서비스와 관련한 분쟁은 서울중앙지방법원을 전속 관할법원으로 합니다.
          </p>

          <v-divider class="my-4" />
          <p class="mb-2 font-weight-bold">{{ $t('auto_key_209') }}</p>
          <p class="text-body-2">
        ...
        <template v-else-if="type === 'privacy'">
          <p class="mb-2">{{ $t('auto_key_210') }}</p>
          <p class="text-body-2 mb-2">
            이메일, 비밀번호, 닉네임, 전화번호, GPS 위치정보
          </p>

          <p class="mb-2">{{ $t('auto_key_211') }}</p>
          <p class="text-body-2 mb-2">
            - 회원 식별 및 인증<br />
            - 위치 기반 중고/알바 매칭<br />
            - 푸시 알림 발송<br />
          ...
          <p class="mb-2">{{ $t('auto_key_212') }}</p>
          <p class="text-body-2 mb-2">
            회원 탈퇴 시 또는 수집 목적 달성 후 파기. 단, 관계법령에 따라 보존 필요 시에는 그에 따릅니다.
          </p>

          <p class="mb-2">{{ $t('auto_key_213') }}</p>
          <p class="text-body-2 mb-2">
      ...
      <v-card-actions class="justify-end">
        <v-btn text @click="emit('update:modelValue', false)">{{ $t('auto_key_174') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
