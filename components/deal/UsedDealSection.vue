<template>
  ...
  <v-card class="pa-4 rounded-lg elevation-1 mt-6">
    ...
    <template v-if="deal.pricingType === 'BIDDING'">
      <div class="text-subtitle-1 font-weight-medium mb-2 color-black">{{ $t('auto_key_186') }}</div>
    ...
    <template v-else>
      <div class="text-subtitle-1 font-weight-medium mb-3 color-black">{{ $t('auto_key_187') }}</div>
...
const submitBid = async () => {
    ...
    try {
        await bidApi.placeBid({
            ...
            nickname: auth.user.nickname || t("auto_key_176")
        });
