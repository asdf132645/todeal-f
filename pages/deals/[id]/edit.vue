<template>
  ...
  <div class="py-6">
    <v-card class="pa-5 rounded-lg" elevation="2">
      <div class="text-h6 font-weight-bold mb-4">{{ $t('auto_key_46') }}</div>
      ...
      <!-- 번역 도우미 -->
      <div class="mb-1 text-caption text-grey-darken-1">{{ $t('auto_key_65') }}</div>
      ...
      <div class="text-caption text-grey-darken-1 mb-1 mt-4">{{ $t('auto_key_68') }}</div>
      <v-text-field
...
const getImageUrl = (item: File | string): string => typeof item === "string" ? item : URL.createObjectURL(item);
...
const updateDeal = async () => {
    ...
    try {
        ...
        snackbar.show(t("auto_key_81"), "success");
        router.push(`/deals/detail/${dealId}`);
    } catch (e) {
        snackbar.show(t("auto_key_82"), "error");
    } finally {
        loading.value = false;
    }
};
